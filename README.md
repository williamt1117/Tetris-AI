...
Air pockets represents how many air tiles are covered by a roof. This evaluation returns negative since air pockets are intentionally avoided.
...
Lines cleared represents how many lines will be fully occupied, resulting in a line clear on that row.
...
Elevation disparity represents how "bumpy" the board is. To evaluate this, the elevation is evaluated at each column. Following this, the absolute value of the difference in adjacent elevations is added for each column. This sum is returned as a negative since elevation desparity is intentionally avoided.
...
Minimize wells represents the amount of holes with a depth of 3 or more. A hole depth is determined with the column elevation mentioned above. A hole's depth is determined as the minimum of the 2 adjacent wall height differences. 
...
Tetris represents a 4-line clear. It is given a fixed return value determined purely by whether a tetris exists or not. If it does, 1 is awarded, else 0.
...
Right well is implemented to promote a well on the side of the board. When tiles fill the right well, a negative score is given relative to the elevation of the right side of the board.
...
Blocks above air is similar to air pockets as it punishes gaps in the board. Blocks above air additionally punishes stacking blocks overtop of an air gap. This is determined by counting the amount of blocks above air tiles and returning the negative of that number.
